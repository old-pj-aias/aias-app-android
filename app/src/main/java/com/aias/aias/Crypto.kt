...
object Crypto {
    ...
    fun savePassword(context: Context, plainPassword: String) {
        ...
        val editor = context.getSharedPreferences("Password", Context.MODE_PRIVATE).edit()
        editor.putString("encrypted_password", encryptedPassword).commit()
    }

    fun loadPassword(context: Context): String? {
        val encryptedPassword = context.getSharedPreferences("Password", Context.MODE_PRIVATE).getString("encrypted_password", null)
